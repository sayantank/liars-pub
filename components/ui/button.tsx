...
const buttonVariants = cva(
	"inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-sm text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				default:
					"shadow-md text-background hover:text-primary bg-primary hover:bg-background border-2 border-primary",
				destructive:
					"bg-destructive text-destructive-foreground hover:bg-destructive/90",
				outline:
					"bg-background border-2 border-primary shadow-md text-primary hover:text-background hover:bg-primary",
				secondary:
					"bg-secondary text-secondary-foreground hover:bg-secondary/80",
				ghost:
					"hover:bg-accent hover:text-accent-foreground border-border border-2",
				link: "text-primary underline-offset-4 hover:underline",
			},
			size: {
				default: "h-10 px-6 py-6 text-lg",
				sm: "h-9 px-3 py-3 sm:py-5",
				lg: "h-11 px-8",
				icon: "h-9 w-9",
			},
		},
